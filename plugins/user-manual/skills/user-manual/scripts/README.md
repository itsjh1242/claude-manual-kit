...
```jsonc
{
  ...
  "annotations": {
    ...
    // 원고의 "화면 구성"과 대조: 이미지(들)와 바로 뒤의 번호 목록이 한 묶음이며,
    // 묶음 안 캡처들의 n 합집합이 정확히 1..(그 목록 항목 수)여야 한다.
    // 이미지마다 목록을 새로 시작하면 각 캡처도 1부터, 이미지 여러 장 뒤에
    // 목록 하나면 n 을 이어 부여(첫 장 1,2 / 둘째 장 3,4). 위반은 build 에러
...
| **"화면 구성" 묶음(이미지+뒤따르는 목록)의 뱃지 n 합집합 ≠ 1..(항목 수)** | **에러** — 누락/중복/범위 밖을 구분해 표시. 뱃지-설명 불일치를 잡는 핵심 검사 |
...
| "주의"가 "방법" 뒤에 있음 | 경고 |
| 캡처가 세로로 길어 높이 상한(500pt)으로 축소됨 | 경고 — clip 으로 좁히거나 화면 분할 권장 |
| 캡처 인쇄 배율이 40% 미만 | 경고 — clip 으로 좁히거나 화면 분할 권장 |
...
- 세로로 긴 그림은 본문 높이의 약 70%(500pt)를 상한으로 자동 축소 — 이미지가
  통째로 다음 페이지로 밀려 반쪽 빈 페이지가 생기는 것을 막는다 (축소 시 경고)
