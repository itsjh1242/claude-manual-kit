...
- `"화면 구성" 목록 ↔ 캡처 뱃지 번호 불일치 (누락/중복/범위 밖)` → 이미지와 바로 뒤
  목록이 한 묶음이다. 묶음 안에서 원고 목록과 명세 badges 의 n 을 대조해 한쪽을
  고친다. 이미지마다 목록을 새로 시작하면 뱃지도 1부터, 이미지 여러 장 뒤 목록
  하나면 n 을 이어 부여(1,2 / 3,4). 뱃지 없는 요소를 목록에 쓰지 않았는지도 확인.
